...
treatNames<-c('1 Peer', 'Low Variance', 'High Variance', 'Skew Near', 'Skew Far')
...
for (treat in 1:6){
	a<-subset(newDat, newDat$treatment==treat)
...
for (treat in 2:5){
	# draw plot
#	par(plt=c(0.1, 0.95, 0.9-(0.1+0.9*(treat-2)/5), 0.9-(0.1+0.9*(treat-1.35)/5)), new=T)
	par(plt=c(0.1, 0.95, 0.95 - 0.9*(treat-1.35)/4, 0.95-0.9*(treat-2)/4), new=T)
	plot(0, type='n', xlim=c(-0.0, 0.45), ylim=c(-0.025,0.3), xlab='', ylab='', axes=FALSE, yaxs='i')
	# add title
#	title(main=treatNames[treat], cex=1.5, line=0.3)
#	axis(1, at=0:4/10, labels=FALSE)
#	if (treat==1)axis(1, at=0:4/10)
	...
	axis(2, at=0:10/20, labels=FALSE)
	axis(2, at=0:10/10)
...
#	if (treat>1) ypos<-(treat-1)*1.6
	...
	for (ca in c(3,5,7)){
		...
		if (ca==5) rangeSize<-distMat[treat,2]-distMat[treat,1]
		if (ca==7) rangeSize<-distMat[treat,3]-distMat[treat,2]
		numberOfBins<- round((rangeSize-widthAtDots)*55)
	...
	#model prediction
	if (treat>1& include_model==T) points(0,ypos+copys_p[1]/n_p, pch=18, col='red')
...
#	rect(-0.005,ypos-0.02,0.005,ypos+0.02, col='dodgerblue', lwd=2, cex=0.2)
...
#	par(plt=c(0.55, 0.95, 1-(0.1+0.9*(treat-1.65)/5), 1-(0.1+0.9*(treat-1.4)/5)), new=T)
	par(plt=c(0.55, 0.95, 0.95-0.9*(treat-1.7)/4, 0.95-0.9*(treat-1.95)/4), new=T)
...
summMat<-matrix(0, nrow=4, ncol=6)
for (tr in 2:5){
	b<-subset(newDat, newDat$treat==tr)
	summMat[tr-1,]<-summary(b$e2)[1:6]
}

freqsCases0<-freqsCases